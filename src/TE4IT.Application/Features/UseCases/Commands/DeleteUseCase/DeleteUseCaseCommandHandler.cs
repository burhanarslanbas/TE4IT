...
public sealed class DeleteUseCaseCommandHandler(
    ...
    IUserPermissionService userPermissionService) : IRequestHandler<DeleteUseCaseCommand, bool>
{
    public async Task<bool> Handle(DeleteUseCaseCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!userPermissionService.CanEditProject(currentUserId, project))
            throw new ProjectAccessDeniedException(module.ProjectId, currentUserId.Value, "Projede kullanım senaryosu silme yetkiniz bulunmamaktadır.");

        writeRepository.Remove(useCase, cancellationToken);
