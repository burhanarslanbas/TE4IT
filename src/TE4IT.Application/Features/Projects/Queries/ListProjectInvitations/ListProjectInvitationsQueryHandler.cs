...
public sealed class ListProjectInvitationsQueryHandler(
    IProjectReadRepository projectRepository,
    IProjectInvitationReadRepository invitationReadRepository,
    IUserInfoService userInfoService,
    ICurrentUser currentUser,
    IUserPermissionService userPermissionService) : IRequestHandler<ListProjectInvitationsQuery, List<ProjectInvitationListItemResponse>>
{
    public async Task<List<ProjectInvitationListItemResponse>> Handle(ListProjectInvitationsQuery request, CancellationToken cancellationToken)
    {
        ...
        if (!isAdmin && (!userRole.HasValue || userRole.Value != ProjectRole.Owner))
            throw new ProjectAccessDeniedException(request.ProjectId, currentUserId.Value, "Davetleri listelemek için Owner yetkisi gereklidir.");
        ...
        var responses = new List<ProjectInvitationListItemResponse>();
        ...
        foreach (var invitation in invitations)
        {
            ...
            responses.Add(new ProjectInvitationListItemResponse
            {
