...
public sealed class CancelProjectInvitationCommandHandler(
    ...
    IUserPermissionService userPermissionService) : IRequestHandler<CancelProjectInvitationCommand, bool>
{
    public async Task<bool> Handle(CancelProjectInvitationCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!isAdmin && (!userRole.HasValue || userRole.Value != ProjectRole.Owner))
            throw new ProjectAccessDeniedException(request.ProjectId, currentUserId.Value, "Daveti iptal etmek için Owner yetkisi gereklidir.");
        ...
        if (invitation.ProjectId != request.ProjectId)
            throw new BusinessRuleViolationException("Bu davet bu projeye ait değil.");

        // 5. Kabul edilmiş davet iptal edilemez
        if (invitation.AcceptedAt != null)
            throw new BusinessRuleViolationException("Kabul edilmiş davet iptal edilemez.");

        // 6. Zaten iptal edilmiş mi kontrol et
        if (invitation.CancelledAt != null)
            throw new BusinessRuleViolationException("Bu davet zaten iptal edilmiş.");
