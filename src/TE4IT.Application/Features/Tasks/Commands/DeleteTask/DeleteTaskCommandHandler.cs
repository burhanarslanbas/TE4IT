...
public sealed class DeleteTaskCommandHandler(
    ...
    IUserPermissionService userPermissionService) : IRequestHandler<DeleteTaskCommand, bool>
{
    public async Task<bool> Handle(DeleteTaskCommand request, CancellationToken cancellationToken)
    {
        ...
        // Erişim kontrolü: Kullanıcının görevi silme yetkisi var mı?
        if (!userPermissionService.CanDeleteTask(currentUserId, task, project))
            throw new ProjectAccessDeniedException(module.ProjectId, currentUserId.Value, "Görevi silme yetkiniz bulunmamaktadır.");

        writeRepository.Remove(task, cancellationToken);
