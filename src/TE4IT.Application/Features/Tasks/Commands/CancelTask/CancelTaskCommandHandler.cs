...
public sealed class CancelTaskCommandHandler(
    ...
    IUserPermissionService userPermissionService) : IRequestHandler<CancelTaskCommand, bool>
{
    public async Task<bool> Handle(CancelTaskCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!userPermissionService.CanEditTask(currentUserId, task, project))
            throw new ProjectAccessDeniedException(module.ProjectId, currentUserId.Value, "Görevi iptal etme yetkiniz bulunmamaktadır.");
