...
public sealed class StartTaskCommandHandler(
    ...
    IUserPermissionService userPermissionService) : IRequestHandler<StartTaskCommand, bool>
{
    public async Task<bool> Handle(StartTaskCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!userPermissionService.CanEditTask(currentUserId, task, project))
            throw new ProjectAccessDeniedException(module.ProjectId, currentUserId.Value, "Görevi başlatma yetkiniz bulunmamaktadır.");
