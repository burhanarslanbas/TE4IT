...
import com.example.vtys.presentation.viewmodel.ProjectDetailViewModel
...
fun ProjectDetailScreen(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(state.project?.title ?: "Proje Detay") },
                navigationIcon = {
                ...
                state.project?.let { project ->
                    LazyColumn(
                        ...
                        item {
                            ...
                            Text(text = project.description ?: "Açıklama yok", style = MaterialTheme.typography.bodyLarge)
                        }

                        item {
                            Text("Modüller", style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.Bold)
                        }

                        items(state.modules) { module ->
                            ModuleCard(module = module, onClick = { onNavigateToModuleDetail(module.id) })
                        }

                        item {
                            Text("Ekip", style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.Bold)
                        }

                        items(state.members) { member ->
                            ListItem(
                                headlineContent = { Text(member.fullName) },
                                supportingContent = { Text(member.roleName) },
                                leadingContent = {
                                    Surface(
...
fun ModuleCard(module: Module, onClick: () -> Unit) {
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            Text(
                text = module.description ?: "Açıklama yok",
                style = MaterialTheme.typography.bodyMedium,
                maxLines = 2
            )
            Spacer(modifier = Modifier.height(8.dp))
            Text("${module.useCaseCount} Use Case", style = MaterialTheme.typography.bodySmall)
        }
    }
}
