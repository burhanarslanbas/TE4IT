...
import com.te4it.mobile.presentation.viewmodel.DashboardViewModel
...
@Composable
private fun DashboardTaskCard(task: com.te4it.mobile.domain.model.Task, onClick: () -> Unit) {
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .clickable(onClick = onClick),
...
@Composable
fun TaskStatusChip(state: com.te4it.mobile.data.network.dto.TaskState?) {
    val (color, text) = when (state) {
        com.te4it.mobile.data.network.dto.TaskState.COMPLETED -> Color(0xFFE8F5E9) to "Tamamlandı"
        com.te4it.mobile.data.network.dto.TaskState.IN_PROGRESS -> Color(0xFFE3F2FD) to "Devam Ediyor"
        com.te4it.mobile.data.network.dto.TaskState.NOT_STARTED -> Color(0xFFF5F5F5) to "Başlamadı"
        com.te4it.mobile.data.network.dto.TaskState.CANCELLED -> Color(0xFFFFEBEE) to "İptal Edildi"
