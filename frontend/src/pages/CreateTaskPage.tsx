/**
 * Create Task Page
 * Route: /projects/:projectId/modules/:moduleId/usecases/:useCaseId/tasks/new
 */
...
import type { UseCase, Module, Project, TaskType, CreateTaskRequest } from '../types';
...
export function CreateTaskPage() {
    ...
    try {
      ...
      if (projectData.status === 'Archived') {
        toast.warning('Proje arşivde', {
          description: 'Bu proje arşivlenmiş. Task oluşturmak için projeyi aktifleştirin.',
        });
      }
      if (moduleData.status === 'Archived') {
        toast.warning('Modül arşivde', {
          description: 'Bu modül arşivlenmiş. Task oluşturmak için modülü aktifleştirin.',
        });
      }
      if (useCaseData.status === 'Archived') {
        toast.warning('Use case arşivde', {
          description: 'Bu use case arşivlenmiş. Task oluşturmak için use case\'i aktifleştirin.',
        });
      }
    } catch (error: any) {
  ...
  const onSubmit = async (data: CreateTaskForm) => {
    if (!useCaseId || !useCase || !module || !project) return;

    // Aktiflik kontrolü
    if (project.status === 'Archived') {
      toast.error('Proje arşivde', {
        description: 'Task oluşturmak için projeyi aktifleştirin.',
      });
      return;
    }
    ...
    if (module.status === 'Archived') {
      toast.error('Modül arşivde', {
        description: 'Task oluşturmak için modülü aktifleştirin.',
      });
      return;
    }
    ...
    if (useCase.status === 'Archived') {
      toast.error('Use case arşivde', {
        description: 'Task oluşturmak için use case\'i aktifleştirin.',
      });
      return;
    }
    ...
    try {
      setSubmitting(true);
      console.log('[CREATE TASK PAGE] Starting task creation');
      ...
      await TaskService.createTask(useCaseId, taskData);

      console.log('[CREATE TASK PAGE] Successfully created task');
      toast.success('Task oluşturuldu');

      // Use case detail sayfasına yönlendir
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 py-12">
        ...
        >
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Yeni Task</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </motion.div>
        ...
        >
          ...
          <p className="text-[#9CA3AF] text-lg">
            <span className="font-semibold text-[#8B5CF6]">{useCase.title}</span> use case'i için yeni bir task oluşturun
          </p>
        </motion.div>
            ...
            <Alert className="bg-[#F59E0B]/10 border-[#F59E0B]/30">
              ...
              <AlertDescription className="text-[#E5E7EB]">
                {project.status === 'Archived' && (
                  <p>
                    Proje arşivde. Task oluşturmak için{' '}
                ...
                {module.status === 'Archived' && (
                  <p>
                    Modül arşivde. Task oluşturmak için{' '}
                ...
                {useCase.status === 'Archived' && (
                  <p>
                    Use case arşivde. Task oluşturmak için{' '}
